...
//小程序色调
export let TypeColor = color[DAY]['#f5222d']

//色调修改
export const colorChange = (status: boolean, styleColor: string, needUpdateStorage: boolean=false) => {
  if(needUpdateStorage) {
    if(status) {  //色调开启
      ...
      if(date) {  //日
        ...
        TypeColor = { ...TypeColor, ...color[NIGHT][styleColor] }
      }
    }else {   //色调关闭
      TypeColor = { ...TypeColor, ...color[DAY][styleColor] }
    }
