...
export const isEqualObj = (targetA, targetB) => {
  ...
  return keysA.some(key => {
    ...
    if (typeA === '[object Object]')
      return isEqualObj(targetA[key], targetB[key]);
    //数字类型中的NaN
    if (
      typeA === '[object number]' &&
      (Number.isNaN(targetA[key]) || Number.isNaN(targetB[key]))
    ) {
...
export const login4Request = async (getUserInfo: any, userRequest: any, customerRequest: any) => {
  const isLogin = await getUserInfo({ prompt: false })
  return isLogin ? customerRequest : userRequest
};
