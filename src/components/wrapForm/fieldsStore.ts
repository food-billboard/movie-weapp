...
class FieldsStore {
  ...
  constructor(ne, ops, fies) {
    ...
    if (!!!ops) {
      ...
      if (typeof ops === 'object') {
        const result = Object.keys(ops).some(opsName => {
          return OPTIONS_NAME.includes(opsName);
        });
        if (result) {
          options = ops;
        } else {
  ...
  hasRules = function(name) {
    ...
    return (
      !!fieldMeta.rules &&
      Array.isArray(fieldMeta.rules) &&
      fieldMeta.rules.length
    );
  };
    ...
    validator.validate(allValues, options, errors => {
      if (errors && errors.length) {
        const errorFields = errors.reduce((acc, error) => {
          ...
          const field = this.fields[errorFieldName];
      ...
      if (callback) {
        callback(errors, this.getFieldsValue(fieldNames));
      }
    });
  };
  ...
  validateFields = function(ns, ops, cbk) {
    const pending = new Promise((resolve, reject) => {
      ...
      //获取验证字段名称集合
      const fieldNames = names ? names : Object.keys(this.fields);
...
export function createFieldsStore(name, options?, fields?) {
  if (typeof name !== 'string')
    throw new Error('必须给定一个名字与表单一一对应');
  if (!!!instance[name]) {
